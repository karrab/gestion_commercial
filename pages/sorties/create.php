...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Détecter le dépassement de post_max_size (PHP vide $_POST et $_FILES sans code d'erreur)
    $content_length = isset($_SERVER['CONTENT_LENGTH']) ? (int)$_SERVER['CONTENT_LENGTH'] : 0;
    $post_max = (int)ini_get('post_max_size') * 1024 * 1024;
    if ($content_length > 0 && $post_max > 0 && $content_length > $post_max) {
        die('<div class="alert alert-danger">Fichier trop volumineux : la taille totale de la requête (' . round($content_length/1024/1024, 1) . ' MB) dépasse post_max_size (' . ini_get('post_max_size') . '). Modifiez php.ini.</div>');
    }
    ...
    if (isset($_FILES['fichier']) && $_FILES['fichier']['error'] !== UPLOAD_ERR_NO_FILE) {
        if ($_FILES['fichier']['error'] !== UPLOAD_ERR_OK) {
            $upload_errors = [
                UPLOAD_ERR_INI_SIZE   => 'Fichier trop volumineux (limite serveur php.ini : upload_max_filesize=' . ini_get('upload_max_filesize') . ').',
                UPLOAD_ERR_FORM_SIZE  => 'Fichier trop volumineux (limite formulaire).',
                UPLOAD_ERR_PARTIAL    => 'Fichier partiellement telecharge.',
                UPLOAD_ERR_NO_TMP_DIR => 'Dossier temporaire manquant (upload_tmp_dir=' . ini_get('upload_tmp_dir') . ').',
            ...
            if (!in_array($ext, $allowed)) {
                ...
                if (!is_dir(UPLOAD_SORTIES_PATH)) {
                    if (!mkdir(UPLOAD_SORTIES_PATH, 0775, true)) {
                        $errors[] = 'Impossible de creer le dossier : ' . UPLOAD_SORTIES_PATH;
                    }
                }
                if (empty($errors)) {
                    if (!is_writable(UPLOAD_SORTIES_PATH)) {
                        $errors[] = 'Dossier non accessible en ecriture : ' . UPLOAD_SORTIES_PATH;
                    } elseif (!is_uploaded_file($_FILES['fichier']['tmp_name'])) {
                        $errors[] = 'Fichier temporaire invalide.';
                    } else {
                        $newname = uniqid() . '_' . time() . '.' . $ext;
                        $dest = UPLOAD_SORTIES_PATH . DIRECTORY_SEPARATOR . $newname;
                        if (move_uploaded_file($_FILES['fichier']['tmp_name'], $dest)) {
                            $fichier = $newname;
                        } else {
                            $last = error_get_last();
                            $detail = $last ? ' [' . $last['message'] . ']' : '';
                            $errors[] = 'Echec de l\'enregistrement du fichier : ' . $dest . $detail;
                        }
                    }
                }
            }
...
// Dans le script $(document).ready(function() {...})
$(document).ready(function() {
    // Initialiser Select2 avec AJAX pour les services
    initServiceSelect('#service_id', 'Sélectionner un service...');

    // Initialiser Select2 pour les employés de base (sans AJAX pour commencer)
    $('#employe_id').select2({
        placeholder: 'Sélectionner un employé...',
        allowClear: true,
        width: '100%',
        dropdownParent: $('#employe_id').parent(),
        language: {
            noResults: function() {
    ...
    }).prop('disabled', true); // Désactiver par défaut

    // Charger employés quand service demandeur change
    $('#service_id').on('change', function() {
        let service_id = $(this).val();

        // Réinitialiser et désactiver le select employe_id
        $('#employe_id').empty().prop('disabled', true);
        
        if (service_id) {
            // Désactiver Select2 temporairement
            $('#employe_id').select2('destroy');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#employe_id').empty().append('<option value="">Sélectionner un employé...</option>');
                    
                    if (data && data.length > 0) {
                        // Ajouter les options
                        $.each(data, function(index, employe) {
                            const text = employe.text || (employe.prenom + ' ' + employe.nom);
                            const option = new Option(text, employe.id, false, false);
                            $('#employe_id').append(option);
                        });
                        
                        // Réactiver le select
                        $('#employe_id').prop('disabled', false);
                        ...
                        $('#employe_id').append('<option value="">Aucun employé dans ce service</option>');
                        $('#employe_id').select2({
                            placeholder: 'Aucun employé disponible',
                            allowClear: false,
                            width: '100%',
                            dropdownParent: $('#employe_id').parent()
                        });
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#employe_id').empty().append('<option value="">Erreur de chargement</option>');
                    $('#employe_id').select2({
            ...
            $('#employe_id').empty().append('<option value="">Veuillez d\'abord sélectionner un service</option>');
            $('#employe_id').prop('disabled', true);
            $('#employe_id').select2({
